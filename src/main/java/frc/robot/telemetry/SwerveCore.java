...
import static frc.robot.telemetry.SwerveCore.Implementation.RUNNYMEDE;
...
public class SwerveCore {
    ...
    public Implementation                                 implementation        = RUNNYMEDE;
    /** The Field! */
    ...
    public static Map<String, SwerveCore.ModuleTelemetry> modules               = new LinkedHashMap<>();

    public SwerveCore.ModuleTelemetry getModule(String name) {
        if (!modules.containsKey(name)) {
            modules.put(name, new SwerveCore.ModuleTelemetry());
        }
    ...
    public void post() {
        if (implementation != Implementation.YAGSL) {
            // Common Swerve
            // DO NOT write to dashboard if YAGSL - writing is built-in and this would conflict
        ...
        if (implementation == RUNNYMEDE) {
            // Runnymede swerve only - YAGSL sets its own values
            for (String name : modules.keySet()) {
                SwerveCore.ModuleTelemetry module = modules.get(name);
